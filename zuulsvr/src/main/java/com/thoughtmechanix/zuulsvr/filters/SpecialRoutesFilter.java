...
public class SpecialRoutesFilter extends ZuulFilter {
	...
	public Object run() {
		...
		AbTestingRoute abTestRoute = getAbRoutingInfo(filterUtils
				.getServiceId());
	...
	private String buildRouteString(String oldEndpoint, String newEndpoint,
			String serviceName) {
		int index = oldEndpoint.indexOf(serviceName);
		String strippedRoute = oldEndpoint.substring(index
				+ serviceName.length());
		System.out.println("Target route: "
				+ String.format("%s/%s", newEndpoint, strippedRoute));
	...
	private HttpResponse forward(HttpClient httpclient, 
			...
			throws Exception {
		Map<String, Object> info = this.helper.debug(verb, uri, headers,
		...
		InputStreamEntity entity = new InputStreamEntity(
				requestEntity,
				contentLength,
				request.getContentType() != null ? ContentType.create(request
						.getContentType()) : null);
